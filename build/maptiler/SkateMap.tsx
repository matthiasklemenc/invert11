...
// --- CONFIGURATION ---
// IMPORTANT: REPLACE THIS WITH YOUR OWN FREE API KEY FROM MAPTILER.COM
...
const SkateMap: React.FC<Props> = ({ path = [], markers = [], center, zoom, className }) => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            map.current.on('error', (e) => {
                ...
                // Only show visible error if it's a fatal map style load error, ignore tile errors (common in dev/quota limits)
    ...
    useEffect(() => {
        if (!map.current || !isLoaded || path.length < 2) return;
