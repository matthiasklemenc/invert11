...
export default function SkateGamePage({ onClose }: { onClose: () => void }) {
    ...
    useEffect(() => {
        ...
        const checkMobile = () => {
            ...
            const isMobileUA = /android|ipad|iphone|ipod|blackberry|iemobile|opera mini/i.test(ua.toLowerCase());
    ...
    if (isMobile && !isLandscape) {
        return (
            <div className="fixed inset-0 z-[100] bg-gray-900 text-white flex flex-col items-center justify-center p-6">
                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-16 h-16 mb-6 animate-pulse text-[#c52323]">
                ...
                <h2 className="text-2xl font-bold uppercase tracking-widest mb-2 text-center">Rotate Device</h2>
                <p className="text-gray-400 text-center mb-8">Please play in landscape mode.</p>
                <button onClick={onClose} className="border border-gray-600 text-gray-400 px-6 py-2 rounded hover:text-white hover:border-white transition-colors">
                    Exit Game
                </button>
             </div>
        );
